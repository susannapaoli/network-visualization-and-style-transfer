...
def guided_gradcam_captum():
    ...
    ##############################################################################
    gbpp = GuidedBackprop(model)
    ggcc = GuidedGradCam(model, conv_module)

    gbp_attr = gbpp.attribute(X_tensor, target=y_tensor)
    ggc_attr = ggcc.attribute(X_tensor, target=y_tensor)
    plt = visualize_attr_maps('Guided GradCAM', X, y, class_names, [gbp_attr, ggc_attr], ['Guided Backprop', 'Guided GradCAM'] )
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return plt
...
def layer_vis_captum():
    ...
    ##############################################################################
    layer_grad = LayerGradCam(model, layer)
    layer_grad_attr = layer_grad.attribute(X_tensor, target=y_tensor, relu_attributions=True)
    layer_grad_attr_sum = layer_grad_attr.mean(axis=1, keepdim=True)
    ...
    plt = visualize_attr_maps('Layers Visualization', X, y, class_names, [layer_grad_attr_sum, layer_cond_attr_sum], ['Layer GradCam', 'Layer Conductance'])
    ##############################################################################
    #                             END OF YOUR CODE                               #
    ##############################################################################
    return plt
