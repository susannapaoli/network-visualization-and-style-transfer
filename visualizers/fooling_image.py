...
class FoolingImage:
    def make_fooling_image(self, X, target_y, model):
        ...
        for it in range(max_iter):
            ...
            con = output[0, target_y]
            if X_fooling_var.grad is not None:
              X_fooling_var.grad.data.zero_()

            con.backward()

            term = learning_rate * X_fooling_var.grad.data / torch.norm(X_fooling_var.grad.data)
            X_fooling_var.data = X_fooling_var.data + term

            _, label = torch.max(output, 1)

            if label.data[0] == target_y:
              break
